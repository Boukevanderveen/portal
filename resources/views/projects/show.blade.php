...
@section('content')
    <div class="grid grid-cols-5 gap-4 ">
        ...
        <div class="col-span-5">
            ...
            <div class="lg:hidden"style="width:100%; overflow: hidden">
                <img src="{{ asset('/images/projects/' . $project->id . '/' . $project->picture . '') }}">
            </div>
        </div>
        <div class="col-span-5 ">
            ...
            @if (!$project->subprojects->isEmpty())
                    ...
                    @foreach ($project->subprojects as $subproject)
                        <div class="col-span-3 lg:col-span-1 max-w-sm bg-white border border-gray-200 rounded-lg shadow =">
                            <div style="width: 384px; height: 255px; overflow: hidden">
                            ...
                            <div class="p-5">
                                ...
                                <a href="{{ URL::route('projects.subprojects.show', [$project, $subproject]) }}"
                    ...
                    @endforeach
                @endif
                @if (!$project->projectposts->isEmpty())
                    <div class="text-3xl col-span-3">
                        Berichten
                    </div>
                    @foreach ($project->projectposts as $projectpost)
                        <div class="col-span-3 lg:col-span-1 max-w-sm bg-white border border-gray-200 rounded-lg shadow =">
                            <div style="width: 384px; height: 255px; overflow: hidden">
                                <img style="width: 400px; height: 300px"
                                    src="{{ asset('/images/projectposts/' . $projectpost->id . '/' . $projectpost->picture . '') }}">
                            </div>
                            <div class="p-5">
                                <h6 class="mb-2 text-2xl font-bold tracking-tight text-gray-900">{{ $projectpost->name }}
                                </h6>
                                <p class="mb-1 text-gray-500 dark:text-gray-400">{{ $projectpost->intro }}</p>
                                <a href="{{ URL::route('projects.projectposts.show', [$project, $projectpost]) }}"
                                    class="inline-flex items-center px-3 py-2 text-sm text-950 bg-[#ffcd00] hover:bg-yellow-400 focus:ring-4 focus:ring-yellow-300 rounded-sm text-sm px-5 py-2.5 me-2 mb-2 dark:focus:ring-yellow-900">
                                    Lees meer
                                    <svg class="w-3.5 h-3.5 ml-2" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                                        fill="none" viewBox="0 0 14 10">
