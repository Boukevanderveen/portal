...
<div class="col-span-5">
    ...
    <nav class="hidden lg:block flex px-5 py-2 text-gray-700 " aria-label="Breadcrumb">
        <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
            ...
            <li class="inline-flex items-center">
            <a href="{{ route('users.show', $user) }}" class="inline-flex items-center text-sm font-medium text-gray-700 hover:text-blue-600 dark:text-gray-400 dark:hover:text-white">
...
<div class="grid grid-cols-4 grid-rows-2 gap-4">
    ...
    E-mail: <a href="mailto:{{$user->email}}">{{$user->email}} <i class="fa fa-envelope" aria-hidden="true"></i></a> 
    @if(!$user->isAdmin)
    ...
    <div class="mt-3 grid grid-cols-3 gap-4">
        ...
        <div class="max-w-sm bg-white border border-gray-200 rounded-lg shadow col-span-3 lg:col-span-1">
            <img style="width: 400px; height: 300px" src="{{ asset('/images/subprojects/'. $subproject->id .'/'. $subproject->picture.'') }}">

                <div class="p-5">
                        <h6 class="mb-2 text-2xl font-bold tracking-tight text-gray-900">{{$subproject->name}}</h6>
                        <a href="{{ URL::route('projects.show', [$subproject->project]); }}" class="font-bold mb-1 text-gray-500 dark:text-gray-400">Project: {{$subproject->project->name}} <i class="fa-solid fa-arrow-right"></i></a>
                        <p class="mb-1 text-gray-500 dark:text-gray-400">{{$subproject->intro}}</p>
                        <a 
                                href="{{ URL::route('projects.subprojects.show', [$subproject->project, $subproject]); }}" 
                                class="inline-flex items-center px-3 py-2 text-sm text-950 bg-[#ffcd00] hover:bg-yellow-400 focus:ring-4 focus:ring-yellow-300 rounded-sm text-sm px-5 py-2.5 me-2 mb-2 dark:focus:ring-yellow-900">
                                Meer informatie
                                <svg class="w-3.5 h-3.5 ml-2" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                                    fill="none" viewBox="0 0 14 10">
