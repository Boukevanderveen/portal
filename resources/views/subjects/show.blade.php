...
<button onclick="window.location='{{ url()->previous() }}'" type="button"
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
            ...
            <li class="inline-flex items-center">
            <a href="{{ route('subjects.show', $subject) }}" class="inline-flex items-center text-sm font-medium text-gray-700 hover:text-blue-600 dark:text-gray-400 dark:hover:text-white">
                <p class="ml-1"> > {{$subject->name}} </p>
            </a>
            </li>
    ...
    <span class="prose">{!!$subject->description!!}</span>
    <span class="text-base mb-1 ">Praktijk uren: {{ $subject->practical_hours }}</span>
    <span class="text-base mb-1 ">Theorie uren: {{ $subject->theory_hours }}</span>
    <span class="text-base mb-1 ">Zelfstudie uren: {{ $subject->selfstudy_hours }}</span>
    <span class="text-base mb-1 ">Docent: {{ $subject->teacher }}</span>
    <span class="text-base mb-1 ">Periode: {{$subject->period}}</span>
