...
@section('content')
    ...
    <nav class="hidden lg:block flex px-5 py-2 text-gray-700 " aria-label="Breadcrumb">
        <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
            ...
            <li class="inline-flex items-center">
            <a href="{{ route('admin.subprojects.create') }}" class="inline-flex items-center text-sm font-medium text-gray-700 hover:text-blue-600 dark:text-gray-400 dark:hover:text-white">
                <p class="ml-1"> > Nieuw subproject </p>
            </a>
            </li>
        </ol>
    </nav>
        ...
        <div class="col-start-1 col-span-5 w-full bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 ">
            <form method="post" action="{{ route('admin.subprojects.store') }}" enctype="multipart/form-data">
                @csrf
                <div class="grid grid-cols-5 gap-4">
                    <div class="col-start-1 col-span-5 lg:col-start-2 lg:col-span-3">
                        ...
                        <div class="mb-3">
                            ...
                            <option @if(NULL != session()->getOldInput("developers") && in_array($user->id, session()->getOldInput("developers"))) selected @endif value="{{$user->id}}">{{$user->name}}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            class="shadow appearance-none border @error('project') border-red-500 @enderror  rounded w-full py-2 px-3 text-gray-700 mb-3 leading-tight focus:outline-none focus:shadow-outline"
                                ...
                                <option @if(old('project') == $project->id) selected @endif value="{{$project->id}}"> 
                                    {{$project->name}}
                                </option>
                                @endforeach
                            </select>
