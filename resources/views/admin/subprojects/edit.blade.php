...
@section('content')
    <div class="grid grid-cols-5 gap-4">
        ...
        <div class="col-start-1 col-span-5 w-full bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 ">
            <form method="post" action="{{ route('admin.subprojects.update', $subproject) }}"
                ...
                <div class="grid grid-cols-5 gap-4">
                    <div class="col-start-1 col-span-5 lg:col-start-2 lg:col-span-3">
                        <div class="mb-3">
                            ...
                            <input value="{{ old('name', $subproject->name) }}"
                        ...
                        <div class="mb-3">
                            ...
                            <select
                                class="js-example-basic-multiple shadow appearance-none border @error('name') border-red-500 @enderror  rounded w-full py-2 px-3 text-gray-700 mb-3 leading-tight focus:outline-none focus:shadow-outline"
                                name="developers[]" multiple="multiple">
                                @foreach ($users as $user)
                                    <option @if (null == session()->getOldInput('developers') &&
                                            App\Models\SubProjects_Users::where(['sub_project_id' => $subproject->id])->where(['user_id' => $user->id])->exists()) selected @endif
                                        @if (null != session()->getOldInput('developers') && in_array($user->id, session()->getOldInput('developers'))) selected @endif value="{{ $user->id }}">
                                        {{ $user->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select
                                ...
                                @foreach ($projects as $project)
                                    <option
                                        @if (old('project') == null && $subproject->project->id == $project->id) selected @elseif(old('project') == $project->id) selected @endif
                                        value="{{ $project->id }}">
                                        {{ $project->name }}
                                    </option>
                                @endforeach
                        ...
                        <div class="mb-3">
                            ...
                            <input value="{{ old('picture') }}"
                                class="shadow appearance-none border @error('picture') border-red-500 @enderror  mb-3 rounded w-full  text-sm leading-tight focus:outline-none focus:shadow-outline"
