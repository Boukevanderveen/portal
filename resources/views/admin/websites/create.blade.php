...
@section('content')
    ...
    <nav class="hidden lg:block flex px-5 py-2 text-gray-700 " aria-label="Breadcrumb">
        <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
            ...
            <li class="inline-flex items-center">
            <a href="{{ route('admin.websites.create') }}" class="inline-flex items-center text-sm font-medium text-gray-700 hover:text-blue-600 dark:text-gray-400 dark:hover:text-white">
                <p class="ml-1"> > Nieuwe website </p>
            </a>
            </li>
        </ol>
    </nav>
                    ...
                    <div class="text-3xl col-start-1 col-span-5 lg:col-start-2 lg:col-span-3">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="text-sm w-full p-2.5 rounded" id="isPublic" name="isPublic">
                                <option value="0" @if(old('isPublic') !== null && old('isPublic') == 0) selected @endif>Publiek</option>
                                <option value="1" @if(old('isPublic') !== null && old('isPublic') == 1) selected @endif >Privé</option>
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select class=".js-example-basic-single text-sm w-full p-2.5 rounded" id="user" name="user">
                                @foreach($users as $user)
                                <option value="{{$user->id}}" @if(old('user') !== null && old('user') == $user->id) selected @endif >{{$user->name}}</option>
                                @endforeach
                            </select>
                            @if ($errors->has('user'))
