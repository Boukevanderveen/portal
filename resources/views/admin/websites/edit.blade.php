...
@section('content')
    <div class="grid grid-cols-5 gap-4">
        ...
        <div class="col-start-1 col-span-5 w-full bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 ">
            <form method="post" onsubmit="doSubmit()" action="{{ route('admin.websites.update', $website) }}"
                ...
                <div class="grid grid-cols-5 gap-4">
                    <div class="text-3xl col-start-1 col-span-5 lg:col-start-2 lg:col-span-3">
                        <div class="mb-3">
                            ...
                            <input value="{{ old('name', $website->name) }}"
                        ...
                        <div class="mb-3">
                            ...
                            <input value="{{ old('link', $website->link) }}"
                        ...
                        <div class="mb-3">
                            ...
                            <select class="text-sm w-full p-2.5 rounded" id="isPublic" name="isPublic">
                                <option value="0"
                                    @if (old('isPublic') !== null && old('isPublic') == 0) selected @else @if (!$website->isPublic && !$website->isPublic) selected @endif
                                    @endif>Publiek</option>
                                <option value="1"
                                    @if (old('isPublic') !== null && old('isPublic') == 1) selected @elseif($website->isPublic) selected @endif>
                                    Privé</option>
                            </select>
                            ...
                            <select class="text-sm w-full p-2.5 rounded" id="user" name="user">
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}"
                                        @if (old('user') !== null && old('user') == $user->id) selected @elseif($user->id == $website->user->id) selected @endif>
                                        {{ $user->name }}</option>
                                @endforeach
                            </select>
                            @if ($errors->has('user'))
