...
<div class="text-3xl col-start-1 col-span-1">
            <div class="grid grid-cols-6 grid-rows-1 gap-4">
                ...
                <div class="col-span-3">
                    <button onclick="window.location='{{ route('admin.projectposts.create') }}'" type="button"
                        ...
                        project bericht</button>
                </div>

                <div class="col-span-6 lg:col-span-3">
                    <form id="searchForm" action="{{ route('admin.projectposts.search') }}">
                        @csrf
                        <div class="flex">
                ...
                <div class="col-span-6 lg:col-span-3">
                    <form method="post" id="filterForm" action="{{ route('admin.projectposts.filter') }}">
                        @csrf

                                <select class="float-right w-full" id="project_select" onchange="this.form.submit()" name="project_filter"> 
            ...
            <table class="w-full text-sm text-left bg-[#3a5757]">
                ...
                <tbody>
                    ...
                    <tr class="bg-white border-b ">
                        ...
                        <td class="px-6 py-4">
                            {{$projectpost->project->name}}
                        </td>
                        <td class="px-6 py-4">
                            {{ $projectpost->created_at->format('d-m-Y')}}
                        </td>

                        <td class="min-w-28">
                            <form method="post" action="{{ route('admin.projectposts.destroy', $projectpost) }}"> @csrf
                                @method('delete')
                            <button type="submit"
                            onclick="return confirm('Weet je zeker dat je {{ $projectpost->name }} wilt verwijderen?')" role="button" class="fa fa-trash float-right mr-5" aria-hidden="true">
