...
@section('content')
    ...
    <button onclick="window.location='{{ url()->previous() }}'" type="button"
                    class="float-left focus:outline-none font-bold text-950 border border-gray-400 rounded-sm text-sm px-5 py-2 me-2">
                    < Terug</button>
...
<div class="grid grid-cols-5 grid-rows-1 gap-0">
                ...
                <div class="col-span-2">
                    <button onclick="window.location='{{ route('admin.subjects.create') }}'" type="button"
                        class="float-right focus:outline-none font-bold text-950 bg-[#ffcd00] hover:bg-yellow-400 focus:ring-4 focus:ring-yellow-300 rounded-sm text-sm px-5 py-2.5 me-2 mb-2 sdark:focus:ring-yellow-900">Nieuw
                        vak</button>
                </div>
                <div class="col-start-1 col-span-3">
                    <form id="searchForm" action="{{ route('admin.subjects.search') }}">
                        @csrf
                        <div class="flex">
                    ...
                    <tbody>
                        @foreach ($subjects as $subject)
                            <tr class="bg-white border-b ">
                                ...
                                <td class="min-w-24">
                                    <form method="post" action="{{ route('admin.subjects.destroy', $subject) }}"> @csrf
                                        @method('delete')
                                        <button type="submit"
                                            onclick="return confirm('Weet je zeker dat je {{ $subject->name }} wilt verwijderen?')"
